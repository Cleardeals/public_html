...
$var_extra = 'commercial-property-list/&sort='.$sortval.'&state='.$state.'&city='.$cityUrl.'&locationname='.$locationnameUrl.'&property_type='.$property_typeUrl.'&bedrooms='.$bedroomsUrl.'&budget_min='.$budget_minUrl.'&budget_max='.$budget_maxUrl;
...
$dbObj->dbQuery="select count(*) as total from ".PREFIX."com_property where for_property='Sell' and status='1' and admin_del='0'"; // for total number of records for paging
...
if(($minrent[1]=='Thousand') && ($maxrent[1]=='Thousand')){
	...
	if(!empty($minrent[0]) && !empty($maxrent[0])){ 
		...
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price_unit='Thousand' and offer_price between $minrent[0] and  $maxrent[0])";

	}else {

	if(!empty($minrent[0])){

		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price>=$minrent[0] and offer_price_unit='Thousand')";	

	}

	if(!empty($maxrent[0])){

		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Thousand')";	
	...
	if(!empty($minrent[0]) && !empty($maxrent[0])){
		...
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price>=$minrent[0] and offer_price_unit='Thousand'  UNION select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Lacs' )";
	...
	if(!empty($maxrent[0])){
		...
		$dbObj->dbQuery.=" or id in (select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Lacs')";
	...
	if(!empty($minrent[0]) && !empty($maxrent[0])){
		...
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price>=$minrent[0] and offer_price_unit='Thousand'  UNION select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Crores' )";
	...
	if(!empty($maxrent[0])){
		...
		$dbObj->dbQuery.=" or id in (select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Crores')";
	...
	if(!empty($minrent[0]) && !empty($maxrent[0])){ 
		...
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price_unit='Lacs' and offer_price between $minrent[0] and  $maxrent[0])";

	}else {

	if(!empty($minrent[0])){

		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price>=$minrent[0] and offer_price_unit='Lacs')";

	}

	if(!empty($maxrent[0])){

		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Lacs')";
	...
	if(!empty($minrent[0]) && !empty($maxrent[0])){
		...
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price>=$minrent[0] and offer_price_unit='Lacs'  UNION select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Crores' )";
	...
	if(!empty($minrent[0]) && !empty($maxrent[0])){ 
		...
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price_unit='Crores' and offer_price between $minrent[0] and  $maxrent[0])";

	}else {

	if(!empty($minrent[0])){
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price>=$minrent[0] and offer_price_unit='Crores')";
	}

	if(!empty($maxrent[0])){
		$dbObj->dbQuery.=" and id in (select property_id from ".PREFIX."com_property_detail where offer_price<=$maxrent[0] and offer_price_unit='Crores')";
	}}
}
}
...
$dbObj->dbQuery="select * from ".PREFIX."com_property where for_property='Sell' and status='1' and admin_del='0'"; // for listing of records
...
<div class="center-section-in">
  <div class="container">
    ...
    <div>
      <ul id="property-tab" role="tablist" class="nav nav-tabs nav-pills flex-column flex-sm-row text-center bg-light border-0 rounded-nav">
        <li class="nav-item flex-sm-fill"> <a id="home-tab" href="<?=HTACCESS_URL?>commercial-property/" class="nav-link border-0 text-uppercase font-weight-bold active btn">For Sell</a> </li>
        <li class="nav-item flex-sm-fill"> <a id="profile-tab" href="<?=HTACCESS_URL?>rent-commercial-property/" class="nav-link border-0 text-uppercase font-weight-bold btn">For rent</a> </li>
      </ul>
      <div class="property-form">
        <div class="tab-content">
          <div class="dropdown">
            <div id="tab1" role="tabpanel" aria-labelledby="home-tab" class="tab-pane fade show active">
              <form action="<?=HTACCESS_URL?>commercialpropertyController.php" method="post" autocomplete="off">
                <input type="hidden" name="mode" value="search_for_sell_listing">
                <div class="row">
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-4 col-4 text-right btn-list"><a  href="<?=HTACCESS_URL?>search-property-thumb/" class="active btn">Residential Properties</a> <a href="<?=HTACCESS_URL?>commercial-property/" class="btn"> <i class="fa fa-th-large" aria-hidden="true"></i></a> <a href="<?=HTACCESS_URL?>commercial-property-list/" class="btn active"> <i class="fa fa-list" aria-hidden="true"> </i></a> </div>
    </div>
    <?php if($cntH>0){?>
    <div class="row">
      ...
      <div class="col-12 row properties-div2 m-0 wow fadeIn">
        ...
        <div class="col-lg-8 col-md-8">
          <div class="properties-name pt-3 pl-3 pr-3 pb-0">
                ...
                <div class="heart heart2">
                  <div class="row m-0">
                    ...
                    <div class="col-lg-3 col-6">
                      ...
                      <?php }else{?>
                      <?php if($favourite==1){?>
                      <a href="<?=HTACCESS_URL?>userController.php?mode=remove_favourite_list&property_id=<?=$dbProperty[$i]['id']?>"> <i class="fa fa-heart" style="color:#e30000;"></i></a>
                      <?php }else{?>
                      <a href="<?=HTACCESS_URL?>userController.php?mode=add_favourite_list&property_id=<?=$dbProperty[$i]['id']?>&id=<?=$favouriteID?>"> <i class="fa fa-heart"></i></a>
                      <?php }}?>
                    </div>
                  </div>
                </div>
            ...
            <h4 class="font-semibold mt-1 mb-3 float-left pl-3"> <a href="<?=HTACCESS_URL?>commercial-property-detail/<?=$dbProperty[$i]['id']?>/<?=$propertyName?>/">
              ...
              <div class="col-md-12">
                <div class="bs-example">
                  <form action="/" method="post" id="accFormmodifyClient<?=$i?>" onSubmit="return chkContact<?=$i?>();" autocomplete="off">
                    <input type="hidden" name="mode" value="property_listing_contact">
...
//alert(1111);
    $.ajax({
        url: "<?=HTACCESS_URL?>commercialpropertyController.php?mode=property_listing_contact",
        data:form_data,
		cache:false,
		async:false,
...
function getOtp(MobileNo){
	...
	//itemCodes = utf8_encode(itemCode);
	 ...
	 $.ajax({
			url:'<?=HTACCESS_URL?>commercialpropertyController.php?mode=get_otp_listing',
