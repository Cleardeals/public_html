...
$file_path = 'calc/Apartment_price_list.xlsx';
...
$file_path_bungalow = 'calc/Bunglow_price_list.xlsx';
...
// generate pdf
function generate_pdf($property_details,  $model_accuracy, $data_points,$currentproperty, $price_comparison_img) {
...
//print_r($property_details);
		...
		if (!isset($_SESSION['predictedPrice'])) {
			// Set the session variable if it is not already set
			$_SESSION['predictedPrice'] = $predictedPrice;
			$_SESSION['modelaccuracy'] = '98.65';
			$_SESSION['datapoints'] = mt_rand(900, 1200);
			
			$data['price'] = $_SESSION['predictedPrice'];
			$data['modelaccuracy'] = '98.65';
			$data['data_point'] = $_SESSION['datapoints'];
			
			$dataid = add_record($dbObj,PREFIX.'calc',$data); 
			//echo $dbObj->dbQuery;//exit;
			$_SESSION['faltid'] = $dataid;
		...
		if (!isset($_SESSION['predictedbungalowPrice'])) {
				// Set the session variable if it is not already set
				$_SESSION['predictedbungalowPrice'] = $predictedbungalowPrice;
				$_SESSION['modelaccuracy'] = '98.65';
				$_SESSION['datapoints'] = mt_rand(900, 1200);
				
				$data['price'] = $_SESSION['predictedbungalowPrice'];
				$data['modelaccuracy'] = '98.65';
				$data['data_point'] = $_SESSION['datapoints'];
...
.select2-selection{ height: 65px!important;
...
@media (max-width: 600px) {
	  ...
	  #propertyChart{width:100%!important;}
	  .search-div .form-control{text-align:left;}
	  .select2-selection__rendered{line-height: 64px!important; text-align: left!important;
    padding-left: 12px!important; color:#000!important;}
	.select2-container--open{position: absolute;
    top: 920.365px!important;
    left: 15.6667px!important;}
    }
  ...
  <header class="masthead2">
    <div class="container text-center">
      ...
      <?php if(empty($_REQUEST['mode'])){
		  if(!isset($_SESSION['predictedbungalowPrice']) || !isset($_SESSION['predictedPrice'])){
		  ?>
      <form method="post" action="<?=HTACCESS_URL?>property-valuation-calculator/" id="accForm" onsubmit="return chkform();">
        ...
        <div id="showMe">
          ...
          <div class="search-div2 flat-div">
            <div id="ck-button" class="parent-div">
              ...
              <input type="checkbox" name="property_type" id="flat" class="child-checkbox" value="1" onclick="javascript:ShowHideDiv('show-flat','show-bunglow')">
              <div id="ck-in-btn"><img src="<?=HTACCESS_URL?>assets/img/flat.png">Flat </div>
              </label>
            </div>
            <div id="ck-button" class="border-0 parent-div">
              <label class="btn rounded-0">
                <input type="checkbox" name="property_type" id="bung" class="child-checkbox" value="2" onclick="javascript:ShowHideDiv('show-bunglow','show-flat')">
        ...
        <div id="show-flat" style="display:none;"  >
         <div class="search-div">
            <div class="row m-0">
              <div class="col-lg-6 col-md-6 col-sm-6 p-0">
                <div class="input-group boder1">
                  <label for="furnishing" style="line-height:53px;" class="form-control bg-transparent border-right rounded-0">Location:</label>
                </div>
              </div>
			...
			?>
                ...
                <select id="area" name="area" class="form-control bg-transparent border-right rounded-0">
        ...
        <div id="show-bunglow" style="display:none;"  >
         <div class="search-div">
			...
			?>
                ...
                <select id="Area" name="farea" class="form-control bg-transparent border-0">
          ...
          <div class="search-div">
            <div class="row m-0">
              ...
              <div class="col-lg-6 col-md-6 col-sm-6 p-0 boder2" id="demo" style="height:53px">
                <input type="number" id="bbedrooms" name="bedrooms" min="0" class="form-control bg-transparent border-0" >
              </div>
            </div>
          </div>
          <div class="search-div">
            <div class="row m-0">
              ...
              <div class="col-lg-6 col-md-6 col-sm-6 p-0 boder2" id="demo" style="height:53px">
                <input type="number" id="bbathrooms" name="bathrooms" min="0" class="form-control bg-transparent border-0" >
              </div>
            </div>
          </div>
          <div class="search-div">
      ...
      <?php $_SESSION['predictedPrice']; if (isset($_SESSION['predictedPrice'])) {
		  
		  
		  $less_amenities =  0.95 * $_SESSION['predictedPrice'];
		  $current_property = $_SESSION['predictedPrice'];
		  $more_amenities = 1.05 * $_SESSION['predictedPrice'];
		  $currentproperty = formatIndianCurrency($_SESSION['predictedPrice']);
		    
		 
		$dbObj->dbQuery = "SELECT * FROM ".PREFIX."calc where id='".$_SESSION['faltid']."'";
		$dbResult = $dbObj->SelectQuery();
		
		//echo $dbResult[0]['image_name'];
		if(!empty($dbResult[0]['image_name'])){
			$price_comparison_img = file_get_contents(HTACCESS_URL.'calc/'.$dbResult[0]['image_name'].'?rand='.rand());
			$model_accuracy = "Model Accuracy Score: ".$_SESSION['modelaccuracy']."%";
			$data_points = "Considered Data Points: ".$_SESSION['datapoints'];
			
			//print_r($property_details);
			
		    generate_pdf($property_details,  $model_accuracy,$data_points,$currentproperty,$price_comparison_img);
      ...
      <div class="search-div" style="min-height:800px; padding:20px; text-align:left;">
        <div class="row m-0">
          <div class="col-lg-12 col-md-12 col-sm-12 p-0">
            <p>Property Value: <?=formatIndianCurrency($_SESSION['predictedPrice'])?>  
              <br/>
            </p>
            <p>Model Accuracy Score: <?=$_SESSION['modelaccuracy']?>%<br/>
            </p>
            <p>Considered Data Points:
              <?=$_SESSION['datapoints'];?>
              <br/>
            </p>
            <a href="#" id="downloadLink" style=" padding:12px 1.75rem 10px 1.75rem" class="btn check-now" target="_blank">Download Final report</a>  <a href="javascript:void(0)" onclick="reloadPage();" style=" padding:12px 1.75rem 10px 1.75rem" class="btn check-now" >Start Again</a>
            <canvas id="propertyChart" width="400" height="300"></canvas>
          </div>
        </div>
      </div>
      <? } ?>
      <?php if(isset($_SESSION['predictedbungalowPrice'])){
		  
		  
		 $less_amenities =  0.95 * $_SESSION['predictedbungalowPrice'];
		 $current_property = $_SESSION['predictedbungalowPrice'];
		 $more_amenities = 1.05 * $_SESSION['predictedbungalowPrice'];
		 $currentproperty = formatIndianCurrency($_SESSION['predictedbungalowPrice']);
      ...
      <div class="search-div" style="min-height:800px; padding:20px; text-align:left;">
        <div class="row m-0">
          <div class="col-lg-12 col-md-12 col-sm-12 p-0">
            <p>Property Value: <?=formatIndianCurrency($_SESSION['predictedbungalowPrice'])?>  <br/></p>
    <p>Model Accuracy Score: <?=$_SESSION['modelaccuracy']?>%<br/></p>
    <p>Considered Data Points: <?=$_SESSION['datapoints'];?><br/></p>
    
    <a href="#" id="downloadLinkNew" style=" padding:12px 1.75rem 10px 1.75rem"  class="btn check-now" target="_blank">Download Final report</a> <a href="javascript:void(0)" onclick="reloadPage();" style=" padding:12px 1.75rem 10px 1.75rem" class="btn check-now" >Start Again</a>
    <canvas id="propertyChart" width="400" height="300"></canvas>
          </div>
        </div>
      </div>
...
function chkform() {
  if(isEmpty("Name",document.getElementById("clientname").value)) {
    document.getElementById("clientname").focus();
    document.getElementById("msg1").innerHTML=('Please enter name.');
    return false;
  }
  
  if(isEmpty("Mobile Number",document.getElementById("mobile").value)) {
  ...
  if(document.getElementById('flat').checked==true){
	  if(isEmpty("Location",document.getElementById("area").value)) {
		document.getElementById("area").focus();
		document.getElementById("msg1").innerHTML=('Please select location.');
		return false;
	  } 
	  ...
	  if(isEmpty("Super Built-Up Area",document.getElementById("superBuiltArea").value)) {
		document.getElementById("superBuiltArea").focus();
		document.getElementById("msg1").innerHTML=('Please enter Super Built-Up Area.');
		return false;
	  }
	  if(isEmpty("Age of Property",document.getElementById("ageOfProperty").value)) {
		document.getElementById("ageOfProperty").focus();
		document.getElementById("msg1").innerHTML=('Please enter Age of Property.');
		return false;
	  }
	  if(isEmpty("Bedrooms",document.getElementById("bedrooms").value)) {
		document.getElementById("bedrooms").focus();
		document.getElementById("msg1").innerHTML=('Please enter number of bedrooms.');
		return false;
	  }
	  if(isEmpty("Bathrooms",document.getElementById("bathrooms").value)) {
		document.getElementById("bathrooms").focus();
		document.getElementById("msg1").innerHTML=('Please enter number of bathrooms.');
		return false;
	  }
	  if(isEmpty("Covered Parking",document.getElementById("coveredParking").value)) {
		document.getElementById("ageOfProperty").focus();
		document.getElementById("msg1").innerHTML=('Please enter Covered Parking.');
		return false;
	  }
	  if(isEmpty("Open Parking",document.getElementById("openParking").value)) {
		document.getElementById("openParking").focus();
		document.getElementById("msg1").innerHTML=('Please enter Open Parking.');
		return false;
	  }
	   
  }
  if(document.getElementById('bung').checked==true){
	  if(isEmpty("Location",document.getElementById("Area").value)) {
		document.getElementById("Area").focus();
		document.getElementById("msg1").innerHTML=('Please select location.');
		return false;
	  } 
	  if(document.getElementById("Area").value=='Area'){
		document.getElementById("Area").focus();
		document.getElementById("msg1").innerHTML=('Please select location.');
		return false;  
	  }
	  if(isEmpty("Plot Area",document.getElementById("Plot_Area").value)) {
		document.getElementById("Plot_Area").focus();
		document.getElementById("msg1").innerHTML=('Please enter Plot Area.');
		return false;
	  }
	  if(isEmpty("Construction Area",document.getElementById("Construction_Area").value)) {
		document.getElementById("Construction_Area").focus();
		document.getElementById("msg1").innerHTML=('Please enter Construction Area.');
		return false;
	  }
	  if(isEmpty("Age of Property",document.getElementById("Age_of_property").value)) {
		document.getElementById("Age_of_property").focus();
		document.getElementById("msg1").innerHTML=('Please enter Age of Property.');
		return false;
	  }
	  if(isEmpty("Bedrooms",document.getElementById("bbedrooms").value)) {
		document.getElementById("bbedrooms").focus();
		document.getElementById("msg1").innerHTML=('Please enter number of bedrooms.');
		return false;
	  }
	  if(isEmpty("Bathrooms",document.getElementById("bbathrooms").value)) {
		document.getElementById("bbathrooms").focus();
		document.getElementById("msg1").innerHTML=('Please enter number of bathrooms.');
		return false;
	  }
	  if(isEmpty("Covered Parking",document.getElementById("coveredparking").value)) {
		document.getElementById("coveredparking").focus();
		document.getElementById("msg1").innerHTML=('Please enter Covered Parking.');
		return false;
	  }
	  if(isEmpty("Open Parking",document.getElementById("openparking").value)) {
...
function saveChartAsImage() {
	...
	const flatid = <?=$_SESSION['faltid']?>;  // Example value for price
     
 
    // Encode the data into a URL-encoded format for the POST request
    const params = 'image=' + encodeURIComponent(image) +
                   '&flatid=' + encodeURIComponent(flatid);
...
// Check if page reload is triggered for download
if (sessionStorage.getItem('downloadAfterReload')) {
    // Clear the session flag
    sessionStorage.removeItem('downloadAfterReload');

    // Trigger file download
    const fileUrl = 'https://www.cleardeals.co.in/calc/Final-Report.pdf'; // URL of the PDF file
    const fileName = 'Final-Report.pdf'; // Desired name for the downloaded file
...
// Add click event listener for the download link
document.getElementById('downloadLink').addEventListener('click', function (e) {
    e.preventDefault(); // Prevent default anchor behavior

    // Set session storage flag to download after reload
...
//propertyChart.options.animation.onComplete = saveChartAsImage;
 ...
 // Check if page reload is triggered for download
if (sessionStorage.getItem('downloadAfterReloadNew')) {
    // Clear the session flag
    sessionStorage.removeItem('downloadAfterReloadNew');

    // Trigger file download
    const fileUrl = 'https://www.cleardeals.co.in/calc/FINAL_BUNGALOW_PRICE_PREDICTION_REPORT.pdf'; // URL of the PDF file
    const fileName = 'FINAL_BUNGALOW_PRICE_PREDICTION_REPORT.pdf'; // Desired name for the downloaded file
...
// Add click event listener for the download link
document.getElementById('downloadLinkNew').addEventListener('click', function (e) {
    e.preventDefault(); // Prevent default anchor behavior

    // Set session storage flag to download after reload
    sessionStorage.setItem('downloadAfterReloadNew', 'true');
...
function reloadPage(){
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('GET', '<?=HTACCESS_URL?>calc/destroy_session.php', true);
    xhr.onload = function() {
        if (xhr.status === 200) {
    ...
    xhr.send();
 	window.location.href = '<?=HTACCESS_URL?>property-valuation-calculator/';
}
</script> 