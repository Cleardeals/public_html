...
$var_extra = 'search-property-list/&sort='.$sortval.'&state='.$state.'&city='.$cityUrl.'&locationname='.$locationnameUrl.'&property_type='.$property_typeUrl.'&bedrooms='.$bedroomsUrl.'&budget_min='.$budget_minUrl.'&budget_max='.$budget_maxUrl;
...
<div class="center-section-in">
                ...
                <div class="row">
                  ...
                  <div class="col-lg-4 col-md-6 col-sm-6 pl-12 pr-12">
                    <select id="multi-select3" multiple="multiple" class="select-css form-control" name="no_of_bedrooms[]">
                      <?php	$bedroomval = str_replace("'",'',$bedrooms);
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-4 col-4 text-right btn-list"> <a  href="<?=HTACCESS_URL?>commercial-property/" class="active btn">Commercial Properties</a> <a href="<?=HTACCESS_URL?>search-property-thumb/" class="btn"> <i class="fa fa-th-large" aria-hidden="true"></i></a> <a href="<?=HTACCESS_URL?>search-property-list/" class="btn active"> <i class="fa fa-list" aria-hidden="true"> </i></a> </div>
    </div>
    <?php if($cntH>0){?>
    <div class="row">
      ...
      <div class="col-12 row properties-div2 m-0 wow fadeIn">
        ...
        <div class="col-lg-8 col-md-8">
          <div class="properties-name pt-3 pl-3 pr-3 pb-0">
                ...
                <div class="heart heart2">
                  <div class="row m-0">
                    ...
                    <div class="col-lg-3 col-6">
                      ...
                      <?php }else{?>
                      <a href="<?=HTACCESS_URL?>userController.php?mode=add_favourite_list&property_id=<?=$dbProperty[$i]['id']?>&id=<?=$favouriteID?>&userId=<?=$_SESSION['user']['userid']?>"> <i class="fa fa-heart"></i></a>
                      <?php }}?>
                    </div>
                  </div>
                </div>
            ...
            <h4 class="font-semibold mt-1 mb-3 float-left pl-3"> <a href="<?=HTACCESS_URL?>property-detail/<?=$dbProperty[$i]['id']?>/<?=$propertyName?>/">
            ...
            <div class="row m-0">
              ...
              <div class="col-md-6 p-0 list"> <a href="<?=HTACCESS_URL?>property-detail/<?=$dbProperty[$i]['id']?>/<?=$propertyName?>/" class="btn themebg text-white theme-btn mr-3">Show Details</a>
                <?php if(!isset($_SESSION['user']['is_login'])) {?>
...
//alert(1111);
    ...
    $.ajax({
        url: "<?=HTACCESS_URL?>propertyController.php?mode=property_listing_contact",
        data:form_data,
		cache:false,
		async:false,
...
function getOtp(MobileNo){
	...
	//itemCodes = utf8_encode(itemCode);
	 $.ajax({
			url:'<?=HTACCESS_URL?>propertyController.php?mode=get_otp_listing',
