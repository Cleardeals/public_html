...
$var_extra = 'rent-commercial-property/&sort='.$sortval.'&state='.$state.'&city='.$cityUrl.'&locationname='.$locationnameUrl.'&property_type='.$property_typeUrl.'&bedrooms='.$bedroomsUrl.'&budget_min='.$budget_minUrl.'&budget_max='.$budget_maxUrl;
...
<div class="center-section-in">
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-4 col-4 text-right btn-list"><a  href="<?=HTACCESS_URL?>search-property-thumb/" class="active btn">Residential Properties</a> <a href="<?=HTACCESS_URL?>rent-commercial-property/" class="btn active"> <i class="fa fa-th-large" aria-hidden="true"></i></a> <a href="<?=HTACCESS_URL?>rent-commercial-property-list/" class="btn"> <i class="fa fa-list" aria-hidden="true"></i></a> </div>
    </div>
    <?php if($cntH>0){?>
    <div class="row">
          ...
          <div class="properties-name">
            ...
            <div>
              ...
              <div class="heart">
                ...
                <?php }else{?>
                <a href="<?=HTACCESS_URL?>userController.php?mode=add_favourite_rent&property_id=<?=$dbProperty[$i]['id']?>&id=<?=$dbFav[0]['id']?>"> <i class="fa fa-heart"></i></a>
                <?php }}?>
              </div>
            </div>
            <div class="clearfix"></div>
...
//alert(1111);
    ...
    $.ajax({
        url: "<?=HTACCESS_URL?>commercialpropertyController.php?mode=rent_property_contact",
        data:form_data,
		cache:false,
		async:false,
...
function getOtp(MobileNo){
	...
	//itemCodes = utf8_encode(itemCode);
	 $.ajax({
			url:'<?=HTACCESS_URL?>commercialpropertyController.php?mode=get_otp_rent_thumb',
