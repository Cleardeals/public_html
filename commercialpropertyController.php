...
if($mode=="search_for_sell"){
	...
	header('Location:'.HTACCESS_URL.'commercial-property/&sort='.$sortval.'&state='.$state.'&city='.$city.'&locationname='.$locationname.'&property_type='.$property_type.'&bedrooms='.$no_of_bedrooms.'&budget_min='.$budget_min.'&budget_max='.$budget_max.'/');
	exit;
}
...
if($mode=="search_for_sell_listing"){
	...
	header('Location:'.HTACCESS_URL.'commercial-property-list/&sort='.$sortval.'&state='.$state.'&city='.$city.'&locationname='.$locationname.'&property_type='.$property_type.'&bedrooms='.$no_of_bedrooms.'&budget_min='.$budget_min.'&budget_max='.$budget_max.'/');
	exit;
}
...
if($mode=="search_for_rent"){
	...
	header('Location:'.HTACCESS_URL.'rent-commercial-property/&sort='.$sortval.'&state='.$state.'&city='.$city.'&locationname='.$locationname.'&property_type='.$property_type.'&bedrooms='.$no_of_bedrooms.'&budget_min='.$budget_min.'&budget_max='.$budget_max.'/');
	exit;
}
...
if($mode=="search_for_rent_listing"){
	...
	header('Location:'.HTACCESS_URL.'rent-commercial-property-list/&sort='.$sortval.'&state='.$state.'&city='.$city.'&locationname='.$locationname.'&property_type='.$property_type.'&bedrooms='.$no_of_bedrooms.'&budget_min='.$budget_min.'&budget_max='.$budget_max.'/');
	exit;
}
...
if($mode=="site_visit"){
	...
	$dbObj->dbQuery="select * from ".PREFIX."com_property where id='".$property_id."'";
	$dbProperty = $dbObj->SelectQuery();

	$dbObj->dbQuery="select * from ".PREFIX."com_property_detail where property_id='".$property_id."'";
	...
	if($_SESSION['prop_site_visit']['otp']==$dbObj->sc_mysql_escape($_REQUEST['time_otp'])){
	if(!empty($dbUser[0]['id']) && !empty($property_id) && !empty($_REQUEST['site_visit_date_time'])){

	$info['user_id'] = $dbUser[0]['id'];
	...
	add_record($dbObj,PREFIX."com_site_visit",$info);
  ...
  <table align="Left" border="0" cellspacing="0" cellpadding="5" width="600" style="font-family:Arial, Helvetica, sans-serif;font-size:13px;border:1px solid #cccccc;border-collapse:collapse">
	...
	echo 1;
	//$_SESSION['siteVisit_msg'] = base64_encode("Your request for arrange a site visit send to admin."); 
	//header('location:'.HTACCESS_URL.'property-detail/'.$dbProperty[0]['url'].'/');
	exit();
	}
	}else{
	echo 2;	
...
if($mode=="ask_a_question"){
	...
	if($_SESSION['prop_ask']['otp']==$dbObj->sc_mysql_escape($_REQUEST['ask_otp'])){
	...
	add_record($dbObj,PREFIX."com_ask_question",$info);
	//echo $dbObj->dbQuery;exit;
...
if($mode=="property_contact"){
	...
	if($_SESSION['prop']['otp']==$dbObj->sc_mysql_escape($_REQUEST['contact_otp'])){
	...
	add_record($dbObj,PREFIX."com_prop_contact",$info);
	//echo $dbObj->dbQuery;exit;
...
if($mode=="property_listing_contact"){
	...
	if($_SESSION['prop_listing']['otp']==$dbObj->sc_mysql_escape($_REQUEST['contact_otp'])){
	if(!empty($property_id) && !empty($_REQUEST['clientid']) && !empty($_REQUEST['name']) && !empty($_REQUEST['email']) && !empty($_REQUEST['mobile_no']) && !empty($_REQUEST['message'])){
...
if($mode=="rent_property_contact"){
	...
	if($_SESSION['prop_rent_thumb']['otp']==$dbObj->sc_mysql_escape($_REQUEST['contact_otp'])){
	if(!empty($property_id) && !empty($_REQUEST['clientid']) && !empty($_REQUEST['name']) && !empty($_REQUEST['email']) && !empty($_REQUEST['mobile_no']) && !empty($_REQUEST['message'])){
...
if($mode=="rent_property_list_contact"){
	...
	if($_SESSION['prop_rent_list']['otp']==$dbObj->sc_mysql_escape($_REQUEST['contact_otp'])){
...
if($mode=="get_otp_listing"){
	...
	//echo $_SESSION['prop_listing']['otp'];
	echo 1;
	}
