...
if($page <= 1){
	...
	$page_link .= "<li class='page-item'><a href=\"$PHP_SELF?mo=$var_extra&set=$prev_set&page=$prevpage\" class=\"page-link prev\"><i class='fa fa-angle-left'></i></a></li>";
}
...
for($i=1;$i<=PAGE_LINKS_PER_PAGE;$i++){
	...
	$stylecolor = ($start_page != $page)? "page-item" : "page-item active";
	
	$page_link .= "<li class=\"".$stylecolor."\"><a href=\"$PHP_SELF?mo=$var_extra&set=$set&page=$start_page\" class=\"page-link\">".$start_page."</a></li>";
...
if($nextpage > $start_page && $set==$max_noof_sets){
	$page_link .= "";
 	...
 	if($nextpage != $page)
 		$page_link .= "<li class='page-item'><a href=\"$PHP_SELF?mo=$var_extra&set=$next_set&page=$nextpage\" class=\"page-link next\"><i class='fa fa-angle-right'></i></a></li>";
}
