...
$max_noof_sets = ceil(($totalrecords / RECORD_PER_PAGE_FAV )/PAGE_LINKS_PER_PAGE);
$cur_page_recs = ($totalrecords - ((PAGE_LINKS_PER_PAGE*RECORD_PER_PAGE_FAV)*($set-1)));
...
for($i=1;$i<=PAGE_LINKS_PER_PAGE;$i++){
	...
	if(($i*RECORD_PER_PAGE_FAV)>=$cur_page_recs) break;
	$start_page++;
}
...
$start_limit = (($page-1)* RECORD_PER_PAGE_FAV);
$page_limit = " LIMIT $start_limit,". RECORD_PER_PAGE_FAV;

$num_limit = ($page-1) * RECORD_PER_PAGE_FAV;
$pagerec = $num_limit;
$lastrec = $pagerec + RECORD_PER_PAGE_FAV;
$pagerec = $pagerec + 1;
