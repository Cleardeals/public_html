...
//echo "abc";exit;
	$error = $_FILES["myfile"]["error"];
   {
	...
	resize_img_new($_FILES['myfile']['tmp_name'],600,$ImageExt,PROPERTY_THUMB_IMAGE_PATH.$NewImageName,$ImageExt); // to resize image and upload it in thumbs folder
	...
	//$info['status'] = "0";
	add_record($dbObj,PREFIX.'com_property_images',$info); 
	//echo $dbObj->dbQuery;exit;

	$dbObj->dbQuery="select * from ".PREFIX."com_property_images where property_id='".$property_id."' order by display_order ASC";
	$order = $dbObj->SelectQuery();

	for($i=0;$i<count((array)$order);$i++){
		$dbObj->dbQuery = "update ".PREFIX."com_property_images set display_order='".($i+1)."' where  property_id='".$property_id."' and id=".$order[$i]['id'];
		$dbObj->ExecuteQuery();
	}
