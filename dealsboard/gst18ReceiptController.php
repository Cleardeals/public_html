...
if($mode=="generatemnr"){
	...
	if(!empty($info['rounding'])){
		$info['rounding'] = $dbObj->sc_mysql_escape($info['rounding']);
	}else{
		$info['rounding'] = 0;	
	}
	...
	if(!empty($id)){ // check requested id to update record
		...
		modify_record($dbObj,PREFIX.'gst18_receipt',$info,'id='.$id); 
		//echo $dbObj->dbQuery;exit;
		
		$msg = base64_encode("Record Modified Successfully ."); // message about action performed
	    header('location:index.php?mo=gst18_receipt&msg='.$msg.'&id='.$id.'&page='.$page.'&set='.$set);
	    exit;
		...
		$rid = add_record($dbObj,PREFIX.'gst18_receipt',$info); 
		//echo $dbObj->dbQuery;

		//$recep_no = 'CD00'.$rid;

		//to update Image status
		//$dbObj->dbQuery = "update ".PREFIX."gst_receipt set invoice_no='".$recep_no."' where id=".$rid;
...
if($mode=="receipt_status"){
	...
	//to update Image status
	$dbObj->dbQuery = "update ".PREFIX."gst18_receipt set status='".$set_val."' where id=".$id;
	$dbObj->ExecuteQuery();
...
if($mode=='delete_receipt'){  // to delete seleted record
	...
	delete_record($dbObj,PREFIX.'gst18_receipt','id='.$id); // to delete record
	//echo $dbObj->dbQuery;exit;

	$msg = base64_encode("Record Successfully Deleted."); // message about action performed
	header('location:index.php?mo=gst18_receipt&id='.$id.'&msg='.$msg);
	exit;
}
?>