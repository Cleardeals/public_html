...
if($mode=="add_update_property"){
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property_detail where property_id='".$id."'";
	...
	if(!empty($_REQUEST['offer_price'])){

		$data['offer_price'] = $dbObj->sc_mysql_escape($_REQUEST['offer_price']);
	...
	// $data['custom_tag'] = $dbObj->sc_mysql_escape($_REQUEST['custom_tag'] ?? "");
	...
	/*if(isset($_FILES['brochure']) && $_FILES['brochure']['size']>0){
		...
		header('location:index.php?mo=add_property&msg='.$msg);
	...
	modify_record($dbObj,PREFIX.'property',$info,'id='.$id);
	...
	modify_record($dbObj,PREFIX.'property_detail',$data,'property_id='.$id);
	...
	add_record($dbObj,PREFIX.'property_detail',$data);
	...
	modify_record($dbObj,PREFIX.'appliances',$adata,'property_id='.$id);
	...
	add_record($dbObj,PREFIX.'appliances',$adata);
	...
	header('location:index.php?mo=property&msg='.$msg);
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property where property_name='".$property_name."' order by id desc";
	...
	$id = add_record($dbObj,PREFIX.'property',$info);
	...
	$dbObj->dbQuery = "SELECT * FROM ".PREFIX."property order by display_order ASC";
	...
	for($k=0;$k<count($dbPages);$k++){
		...
		$dbObj->dbQuery = "update ".PREFIX."property set display_order='".($k+1)."' where id=".$dbPages[$k]['id'];
...
if($mode=='delete_property'){  //to delete seleted record
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property_detail where property_id=".$id[$i];
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property_images where property_id=".$id[$i];
	...
	delete_record($dbObj,PREFIX.'property_images','property_id='.$dbResult[$j]['id']); // to delete record
	...
	delete_record($dbObj,PREFIX.'property','id='.$id[$i]); // to delete record

	//echo $dbObj->dbQuery;exit;

	

	delete_record($dbObj,PREFIX.'property_detail','property_id='.$id[$i]); // to delete record
	...
	$dbProperty = $dbObj->SelectQuery();
		...
		for($i=0;$i<count($dbProperty);$i++){
			...
			$dbObj->dbQuery = "update ".PREFIX."property set display_order='".($i+1)."' where id=".$dbProperty[$i]['id'];
...
if($mode=='delete_single_property'){  // to delete seleted record
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property_images where property_id=".$id;
	...
	delete_record($dbObj,PREFIX.'property_images','property_id='.$id); // to delete record
	...
	delete_record($dbObj,PREFIX.'property_detail','property_id='.$id); // to delete record

	//echo $dbObj->dbQuery;exit;

	

	delete_record($dbObj,PREFIX.'appliances','property_id='.$id); // to delete record

	//echo $dbObj->dbQuery;exit;

	

	delete_record($dbObj,PREFIX.'property','id='.$id); // to delete record
...
if($mode=='delete_admin_property'){  // to delete seleted record
	...
	$dbObj->dbQuery = "update ".PREFIX."property set admin_del='1' where id=".$id;
...
if($mode=='property_restore'){  // to delete seleted record
	...
	$dbObj->dbQuery = "update ".PREFIX."property set admin_del='0' where id=".$id;
...
if($mode=='delete_deleted_property'){  // to delete seleted record
	...
	header('location:index.php?mo=deleted_property&id='.$id.'&msg='.$msg);
...
/*if ($mode == "change_property_order"){
	...
	for($i=0;$i<count($tdata);$i++){
		...
		$query = "UPDATE ".PREFIX."property SET display_order = " . $list . " WHERE id = ".$tdata[$i];
...
if ($action == "change_property_order"){
	...
	$query = "UPDATE ".PREFIX."property SET display_order = " . $listingCounter . " WHERE id = " . $recordIDValue;
...
if ($action == "change_property_images_order"){
	...
	$query = "UPDATE ".PREFIX."property_images SET display_order = " . $listingCounter . " WHERE property_id='".$property_id."' and id = " . $recordIDValue;
...
<?php
	...
	$dbObj->dbQuery="select * from ".PREFIX."property_images WHERE property_id='".$property_id."'"; // to show all records
    ...
    <li id="recordsArray_<?=$dbImages[$i]['id']?>">
      ...
      <table background="#FFF" class="mws-datatable-fn mws-table" id="sample_editable_1">
        ...
        <tr style="background-color:<?=$bgcolor?>" id="move">
          ...
          <a href="index.php?mo=property_images&pid=<?=$property_id?>&property_id=<?=$dbImages[$i]['id']?>">
...
if($mode=="update_property_images"){ 
	...
	header('location:index.php?mo=property_images&property_id='.$property_id.'&id='.$id.'&msg='.$msg);
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property_images WHERE id='".$id."'";
	...
	modify_record($dbObj,PREFIX.'property_images',$info,'id='.$id); 
	...
	header('location:index.php?mo=property_images&property_id='.$property_id.'&msg='.$msg);
...
if($mode=="front_status"){
	...
	$dbObj->dbQuery = "update ".PREFIX."property_images set front_status='".$set_val."' where id=".$id;
...
if($mode=="pro_image_status"){
	...
	$dbObj->dbQuery = "update ".PREFIX."property_images set status='".$set_val."' where id=".$id;
...
if($mode=="delete_property_images"){
	...
	$id = $_REQUEST['id'] ?? ""; // array of selected record ids to delete
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property_images where id=".$id[$i];
	...
	delete_record($dbObj,PREFIX.'property_images','id='.$id[$i]); // to delete record
	...
	$dbObj->dbQuery = "select * from ".PREFIX."property_images where id='".$id."' order by display_order asc";
	...
	for($k=0;$k<count((array)$dbResult1);$k++){
		...
		$dbObj->dbQuery = "update ".PREFIX."property_images set display_order='".($k+1)."' where property_id='".$property_id."' and id=".$dbResult1[$k]['id'];
...
{	
...

	{
		...
		{ 
			...
			$custom_tag = mysqli_real_escape_string($dbObj->connection, $excel->sheets[$i][CELLS][$j][97]);
			...
			$dbObj->dbQuery = "select * from ".PREFIX."property where property_name='".$propertyName."' order by id desc";
...
if($mode=="search_property"){

	$search_text = trim($dbObj->sc_mysql_escape($_REQUEST['search_text'] ?? ""));

	$search_text = str_replace(' ','-',$search_text);

	header('Location:index.php?mo=property&search_text='.$search_text);
