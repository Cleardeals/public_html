...
if($mode=="generatemnr"){
	...
	if(!empty($id)){ // check requested id to update record
		...
		modify_record($dbObj,PREFIX.'gst_receipt',$info,'id='.$id); 
		//echo $dbObj->dbQuery;exit;
		
		$msg = base64_encode("Record Modified Successfully ."); // message about action performed
	    header('location:index.php?mo=gst_receipt&msg='.$msg.'&id='.$id.'&page='.$page.'&set='.$set);
	    exit;
		...
		$rid = add_record($dbObj,PREFIX.'gst_receipt',$info); 
		//echo $dbObj->dbQuery;
		//$recep_no = 'CD00'.$rid;
...
if($mode=="receipt_status"){
	...
	//to update Image status
	$dbObj->dbQuery = "update ".PREFIX."gst_receipt set status='".$set_val."' where id=".$id;
	$dbObj->ExecuteQuery();
...
if($mode=='delete_receipt'){  //to delete seleted record
	...
	delete_record($dbObj,PREFIX.'gst_receipt','id='.$id); // to delete record
	//echo $dbObj->dbQuery;exit;
	
	$msg = base64_encode("Record Successfully Deleted."); // message about action performed
	header('location:index.php?mo=gst_receipt&id='.$id.'&msg='.$msg);
	exit;	
}
?>