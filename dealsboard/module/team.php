...
$var_extra = "team"; // to enable page link
...
/*$dbObj->dbQuery="select count(*) as total from ".PREFIX."team where id!=''"; // for total number of records for paging
...
*/
$dbObj->dbQuery="select * from ".PREFIX."team where id!=''"; // for listing of records
/*if(empty($view)){
...
$cntH = count((array)$dbTeam);
$list = $dbTeam[0]['display_order'];
?>
<style>
#contentLeft {
	width: 100%;
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <?php } ?>
              <div class="row">
                <div  class="text-left col-md-6"> <a href="index.php?mo=add_team" class="btn waves-effect waves-light btn-rounded btn-primary"> Add New</a></div>
                <div class="col-md-6 text-right">
                  <p style="padding-top:5px;color:#F00">Change display order work on view all entries**
                    <?=count((array)$dbTeam)?>
                  </p>
                </div>
              </div>
              <div class="table-responsive mt-10">
                <form action="teamController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_team"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbTeam)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbTeam[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbTeam[$i]['id']?>">
                          </span></td>
                        <td id="move" width="10%"><?=$dbTeam[$i]['name']?></td>
                        <td id="move" width="10%"><?=$dbTeam[$i]['designation']?></td>
                        <td id="move" width="10%"><img src="../cms_images/team/thumb/<?=$dbTeam[$i]['image']?>" width="100"></td>
                        <td width="5%"><?=$dbTeam[$i]['display_order']?></td>
                        <td width="5%"><input type="checkbox" id="s<?=$i+1?>" <?=($dbTeam[$i]['status']==1)?'checked':''?> onClick="team_status(<?=$dbTeam[$i]['id']?>,this.value,this.checked)" value="<?=$dbTeam[$i]['id']?>"></td>
                        <td width="5%"><a href="index.php?mo=add_team&id=<?=$dbTeam[$i]['id']?>" class="text-primary">Edit</a></td>
                      </tr>
                      <?php }?>
                    </tbody>
                    <tfoot>
...
function team_status(id,value,is_check){
	var setval=(is_check==true)?1:0;
	$.ajax({
		url:'teamController.php?mode=team_status',
