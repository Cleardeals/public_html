...
$var_extra = "services_enquiry"; // to enable page link
...
$dbObj->dbQuery="select * from ".PREFIX."service_enquiry"; // for listing of records
...
$dbEnquiry = $dbObj->SelectQuery();
$cntH = count((array)$dbEnquiry);
?>
<style>
#contentLeft {
	width: 100%;
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <?php } ?>
              <h3>Services Enquiry</h3>
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_service_enquiry"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbEnquiry)?>" />
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbSubscribe[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbEnquiry[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=$dbEnquiry[$i]['service']?></td>
                        <td width="10%"><?=$dbEnquiry[$i]['name']?></td>
                        <td width="10%"><?=$dbEnquiry[$i]['email']?></td>
                        <td width="10%"><?=$dbEnquiry[$i]['mobile_no']?></td>
                        <td width="10%"><?=$dbEnquiry[$i]['city']?></td>
                        <td width="10%"><?=$dbEnquiry[$i]['enq_detail']?></td>
                        <td width="5%"><?=$dbEnquiry[$i]['current_datetime']?></td>
                      </tr>
                      <?php }?>
                    </tbody>
                    <tfoot>
