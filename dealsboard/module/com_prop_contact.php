...
$dbObj->dbQuery="select * from ".PREFIX."com_prop_contact"; // for listing of records
...
$dbPropContact = $dbObj->SelectQuery();
$cntH = count((array)$dbPropContact);
?>
<style>
#contentLeft {
	width: 100%;
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <?php } ?>
              <h3>Property Contact Us</h3>
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_prop_contact"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbPropContact)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      <?php for($i=0;$i<$cntH;$i++){ 
					    ...
					    $dbObj->dbQuery="select * from ".PREFIX."property where id='".$dbObj->sc_mysql_escape($dbPropContact[$i]['property_id'])."'";
						$dbProperty = $dbObj->SelectQuery();

						$dbObj->dbQuery="select * from ".PREFIX."property_detail where property_id='".$dbObj->sc_mysql_escape($dbPropContact[$i]['property_id'])."'";
						$dbPropertyDetail = $dbObj->SelectQuery();
					  ?>
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbPropContact[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbPropContact[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=$dbPropContact[$i]['clientid']?></td>
                        <td width="10%"><?=$dbPropContact[$i]['name']?></td>
                        <td width="10%"><?=$dbPropContact[$i]['email']?></td>
                        <td width="10%"><?=$dbPropContact[$i]['mobile_no']?></td>
                        ...
                        <td width="10%"><?=$dbPropContact[$i]['message'] ?? ""?></td>
                        <?php
							$datef = $dbPropContact[$i]['current_datetime'];
