...
$dbObj->dbQuery="select * from ".PREFIX."blog_contact_us"; // for listing of records
...
$dbRequest = $dbObj->SelectQuery();
$cntH = count((array)$dbRequest);
?>
<style>
#contentLeft {
	width:100%;
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <?php } ?>
              <h3>Blog Contact Us</h3>
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_req_call_back"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbRequest)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        ...
                        <td width="10%"><?=$dbRequest[$i]['name']?></td>
                        <td width="10%"><?=$dbRequest[$i]['email']?></td>
                        <td width="10%"><?=$dbRequest[$i]['city']?></td> 
                        <td width="10%"><?=$dbRequest[$i]['mobile']?></td>
                        <td width="10%"><?=$dbRequest[$i]['service']?></td>
                        <?php
							$datef = $dbRequest[$i]['created_at'] ?? "";
                 ...
                 <table id="testTable" class="display nowrap table table-hover table-striped table-bordered" width="100%" style="display:none;">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        <td width="10%"><?=$dbRequest[$i]['name']?></td>
                        <td width="10%"><?=$dbRequest[$i]['email']?></td>
                        <td width="10%"><?=$dbRequest[$i]['mobile_no']?></td>
                        <td width="10%"><?=$dbRequest[$i]['message']?></td>
                        <?php
							$datef = $dbRequest[$i]['current_datetime'] ?? "";
							$appoin_dates = explode(" ",$datef);
							$date = $appoin_dates[0] ?? "";
							$time = $appoin_dates[1] ?? "";
							$dates = date('d/m/Y',strtotime($date));
