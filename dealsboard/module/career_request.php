...
$dbObj->dbQuery="select * from ".PREFIX."career_request"; // for listing of records
...
$dbCareerRequest = $dbObj->SelectQuery();
$cntH = count((array)$dbCareerRequest);
?>
<style>
#contentLeft {
	width:100%;
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <?php } ?>
              <h3>Career Request</h3>
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_career_req"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbCareerRequest)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        ...
                        <span id="<?=$dbSubscribe[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbCareerRequest[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=$dbCareerRequest[$i]['applied_for']?></td>
                        <td width="10%"><?=$dbCareerRequest[$i]['name']?></td>
                        <td width="10%"><?=$dbCareerRequest[$i]['email']?></td>
                        <td width="10%"><?=$dbCareerRequest[$i]['mobile_no']?></td>
                        <td width="10%"><?=$dbCareerRequest[$i]['job_desc']?></td>
                        <td width="10%"><a href="../cms_images/career/<?=$dbCareerRequest[$i]['resume']?>" download>
						<?=$dbCareerRequest[$i]['resume']?></a></td>
                        <?php
							$datef = $dbCareerRequest[$i]['current_datetime'];
							$appoin_dates = explode(" ",$datef);
							$date = $appoin_dates[0];
							$time = $appoin_dates[1];
							$dates = date('d/m/Y',strtotime($date));
						 ?>
                        <td width="5%"><?=$dates.' '.$time?></td>
                      </tr>
                      <?php }?>
                    </tbody>
                    <tfoot>
