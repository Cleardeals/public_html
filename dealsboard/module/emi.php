...
$var_extra = "emi"; // to enable page link
...
$dbObj->dbQuery="select * from ".PREFIX."home_loan_enquiry where clcType='emi'"; // for listing of records
...
$dbEmi = $dbObj->SelectQuery();
$cntH = count((array)$dbEmi);
?>
<style>
#contentLeft {
	width:100%;
            ...
            <div class="card-body">
              ...
              <div class="table-responsive mt-10">
                <form action="homeloanController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_emi"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbEmi)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    <thead>
                      <tr>
                        ...
                        <th width="10%">Total Amount Payble</th>
                        <th width="10%">Date/Time</th>
                      </tr>
                    </thead>
                    <tbody id="changes">
                      <?php for($i=0;$i<$cntH;$i++){ 
					  if(!empty($dbEmi[$i]['name']) && !empty($dbEmi[$i]['email'])){
					  ?>
                      <tr>
                        <td class="text-center" width="5%">
                        <span id="<?=$dbEmi[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbEmi[$i]['id']?>">
                          </span></td>
                          <td width="10%"><?=$dbEmi[$i]['name']?></td>
                        <td width="10%"><?=$dbEmi[$i]['email']?></td>
                        <td width="10%"><?=$dbEmi[$i]['mobile_no']?></td>
                        <td width="10%"><?=$dbEmi[$i]['emi_purpose']?></td>
                        <td width="10%"><?=$dbEmi[$i]['loan_eligibility'] ?? ""?></td>
                        <td width="10%"><?=$dbEmi[$i]['years']?></td>
                        <td width="10%"><?=$dbEmi[$i]['rate']?></td>
                        <td width="10%"><?=$dbEmi[$i]['monthly_emi'] ?? ""?></td>
                        <td width="10%"><?=$dbEmi[$i]['intrest'] ?? ""?></td>
                        <td width="10%"><?=$dbEmi[$i]['totalPay'] ?? ""?></td>
                        <?php
							$datef = $dbEmi[$i]['currentDatetime'];
