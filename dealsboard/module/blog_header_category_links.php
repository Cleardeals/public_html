...
<body class="fix-header card-no-border fix-sidebar">
                     ...
                     <div class="row accordion" id="accordion1">
                        <div class="col-md-12">
                           <div class="card">
                              ...
                              Add/Update Header Category Links<i class="fa accrd-controller fa-caret-down pull-right"></i></a>
                              <div id="collapseOne" class="card-body collapse show border font-13" data-parent="#accordion1">
                                 <!-- Trigger the modal with a button -->
                                 <button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal" >Add Category Link</button>
                                 <input type="hidden" name="mode" value="add_update_blog_hader_links"/> 
                                 <?php for($i=0;$i<$cntH;$i++){?>
                                 <div class="row">
                                    <div class="col-md-12">
                                       <hr>
                                       <h6 style="text-align: center;">Category Name : <?=$dbCategory[$i]['name']?></h6>
                                       <hr>
                                       <?php
                                          $dbObj->dbQuery="select * from ".PREFIX."blog_sub_category where category_id='".$dbCategory[$i]['id']."'"; 
                                       ...
                                       <div class="row">
                                          <div class="col-md-4">
                                             <label>Name</label>
                                             <input type="hidden" name="info[id][<?=$dbCategoryLinks[$j]['category_id']?>][]" value="<?=$dbCategoryLinks[$j]['id']?>">
                                             <input class="form-control" type="text" name="info[name][<?=$dbCategoryLinks[$j]['category_id']?>][]" id="name" placeholder="Category Name" value="<?=$dbCategoryLinks[$j]['name']?>"> 
                                          </div>
                                          <div class="col-md-4">
                                             <label>Image</label>
                                             <input class="form-control" type="file" name="image[<?=$dbCategoryLinks[$j]['category_id']?>][]" > 
                                          </div>
                                          <div class="col-md-2">
                                             <img src="<?=HTACCESS_URL?>cms_images/blog/category/<?=$dbCategoryLinks[$j]['image']?>" width="100" height="100">
                                          </div>
                                          <div class="col-md-2">
                                             <a  data-id="<?=$dbCategoryLinks[$j]['id']?>"  class="btn waves-effect btn-sm waves-light btn-warning mr-auto delete_button" style="color:#fff;">
                                             Delete</a> 
                                          </div>
                                       </div>
                                       <hr>
      ...
      <div class="modal-dialog">
         ...
         <div class="modal-content">
            <form action="blogController.php" method="POST">
               <input type="hidden" name="mode" value="delete_single_category_link"/>
               <input type="hidden" name="info[delete_id]" id="delete_id" /> 
               <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                  <h4 class="modal-title">Delete Header Category Link</h4>
               </div>
   ...
   <div id="myModal" class="modal fade" role="dialog">
      <div class="modal-dialog">
         <!-- Modal content-->
         <div class="modal-content">
            <form action="blogController.php" method="post"  enctype='multipart/form-data'>
               <input type="hidden" name="mode" value="add_new_blog_hader_link"/> 
               <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                  <h4 class="modal-title">Add Header Category Links</h4>
               </div>
               <div class="modal-body">
                  <h6>Blog Sub Category</h6>
                  <select name="info[category_id]" class="form-control" >
                     <?php for($i=0;$i<$cntH;$i++){?> 
                     <option value="<?=$dbCategory[$i]['id']?>" > <b> <?=$dbCategory[$i]['name']?> </b></option>
                     <?php }?>
                  </select>
                  <br>
                  <br>
                  <div class="row">
                     <div class="col-md-12">
                        ...
                        <input class="form-control" type="text" name="info[name]" id="name" placeholder="Category Name" > 
                     </div>
                  </div>
                  <br>
                  <div class="row">
