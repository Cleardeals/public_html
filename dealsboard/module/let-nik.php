...
$dbObj->dbQuery="select * from ".PREFIX."find_property where id!=''"; // for listing of records
            ...
            <div class="card-body">
              ...
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  ...
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbFind)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="">
                      <?php for($i=0;$i<$cntH;$i++){
						...
						$dbObj->dbQuery="select * from ".PREFIX."user_detail where id='".$dbObj->sc_mysql_escape($dbFind[$i]['user_id'])."'"; 
						$dbUser = $dbObj->SelectQuery();
						?>
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbFind[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbFind[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=$dbUser[0]['name'] ?? ""?></td>
                        <td width="8%"><?=$dbFind[$i]['range'] ?? ""?></td>
                        <td width="8%"><?=$dbFind[$i]['city'] ?? ""?></td>
                        <td width="8%"><?=$dbFind[$i]['location'] ?? ""?></td>
                        <td width="20%"><?=$dbFind[$i]['message'] ?? ""?></td>
                        <td width="8%"><a href="index.php?mo=find-property&id=<?=$dbFind[$i]['user_id']?>" class="text-primary">View</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="userController.php?mode=delete_letnik_property&id=<?=$dbFind[$i]['id']?>" class="text-primary" data-confirm="Are you sure you want to delete?"><i class="fa fa-trash"></i></a></td>
                      </tr>
                      <?php }?>
                    </tbody>
                    <tfoot>
