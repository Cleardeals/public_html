...
$var_extra = "payment_receipt"; // to enable page link
...
$dbObj->dbQuery="select * from ".PREFIX."payment_receipt where user_id='".$user_id."'"; // for listing of records
...
$cntH = count((array)$dbReceipt);

$dbObj->dbQuery="select * from ".PREFIX."payment_receipt where id='".$id."'"; // for listing of records
  ...
  <div class="page-wrapper"> 
    ...
    <div class="container-fluid"> 
      ...
      <form action="userController.php" method="post" id="accForm" enctype="multipart/form-data">
        <input type="hidden" name="mode" value="payment_receipt" />
        ...
        <div class="row" id="main">
          <div class="col-md-12" id="content">
            <div class="row accordion" id="accordion1">
              <div class="col-md-12">
                <div class="card"> <a class="card-header collapsed in card-title" data-toggle="collapse" href="#collapseOne"> Payment Receipt <i class="fa accrd-controller fa-caret-down pull-right"></i></a>
                  <div id="collapseOne" class="card-body collapse show border font-13" data-parent="#accordion1">
                    ...
                    <a href="../cms_images/payment_receipt/<?=$dbRes[0]['upload_file']?>" style="color:#00F;"> Download</a>
                    <?php } ?>
                    <input class="form-control" type="file" name="upload_file" id="file" accept=".pdf">
                    <br/>
                    <p style="font-size:12px;color:#F00;margin-top:10px;"> Pdf (File Will Accept)</p>
                    <br>
                ...
                <form action="userController.php" method="post" id="hostForm">
                  ...
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbReceipt[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbReceipt[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=$dbReceipt[$i]['title']?></td>
                        <td width="10%"><a href="../cms_images/payment_receipt/<?=$dbReceipt[$i]['upload_file']?>">
                          <?=$dbReceipt[$i]['upload_file']?>
                          </a></td>
                        <td width="10%"><input type="checkbox" id="s<?=$i+1?>" <?=($dbReceipt[$i]['status']==1)?'checked':''?> onClick="receipt_status(<?=$dbReceipt[$i]['id']?>,this.value,this.checked)" value="<?=$dbReceipt[$i]['id']?>"></td>
                        <!--<td width="10%"><?=$dbReceipt[$i]['display_order']?></td>-->
                        <td width="8%"><a href="index.php?mo=payment_receipt&user_id=<?=$dbReceipt[$i]['user_id']?>&id=<?=$dbReceipt[$i]['id']?>" class="text-primary">Edit</a></td>
                      </tr>
                      <?php }?>
                    </tbody>
                    <tfoot>
...
function ckhform(){
	...
	<?php if(empty($dbRes[0]['upload_file'])) {?>
	if(isEmpty("Payment Receipt",document.getElementById("file").value)){
...
function receipt_status(id,value,is_check){
	...
	$.ajax({
		url:'userController.php?mode=receipt_status',
