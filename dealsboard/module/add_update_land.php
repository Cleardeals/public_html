...
//$dbObj->dbQuery="select * from ".PREFIX."city where id='".$dbland[0]['id']."'";
  ...
  <div class="page-wrapper"> 
    ...
    <div class="container-fluid"> 
      ...
      <form action="contentController.php" method="post" id="accForm" enctype="multipart/form-data">
        <input type="hidden" name="mode" value="add_update_land" />
        <input type="hidden" name="id" value="<?=$id?>" />
        <div class="row" id="main">
          <div class="col-md-9" id="content">
            <div class="row accordion" id="accordion1">
              <div class="col-md-12">
                <div class="card"> <a class="card-header collapsed in card-title" data-toggle="collapse" href="#collapseOne"> Add/Update land 
                <i class="fa accrd-controller fa-caret-down pull-right"></i></a>
                  <div id="collapseOne" class="card-body collapse show border font-13" data-parent="#accordion1">
                    <div class="row">
                      <div class="col-md-6">
                        <h6>City</h6>
                        <?php if(!empty($dbland[0]['city'])){?>
                        <select class="form-control" name="info[city]" id="city">
                          <option value="">City</option>
                          <?php for($i=0;$i<count((array)$City);$i++){?>
                          <option value="<?=$City[$i]['city_name']?>" <?=($dbland[0]['city']==$City[$i]['city_name'])?'selected':''?>>
                          <?=$City[$i]['city_name']?>
                          </option>
                          <?php }?>
                        </select>
                      ...
                      <div class="col-md-6">
                        <h6>land</h6>
                        <input class="form-control" type="text" name="info[land]" id="land" placeholder="land" value="<?=$dbland[0]['land']?>">
                        <br>
                        <br>
                      </div>
                      <div class="col-md-6">
                        <h6>Price Range (Min.) Rs./sq.ft</h6>
                        <input class="form-control" type="text" name="info[price_min_sq]" id="price_min_sq" placeholder="Price Range (Min.) Rs./sq.ft" value="<?=$dbland[0]['price_min_sq']?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                        <br>
                        <br>
                      </div>
                      <div class="col-md-6">
                        <h6>Price Range (Max.) Rs./sq.ft</h6>
                        <input class="form-control" type="text" name="info[price_max_sq]" id="price_max_sq" placeholder="Price Range (Max.) Rs./sq.ft" value="<?=$dbland[0]['price_max_sq']?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                        <br>
                        <br>
                      </div>
                      <div class="col-md-6">
                        <h6>Price Range (Min.) Rs./sq.yd</h6>
                        <input class="form-control" type="text" name="info[price_min_sq_yrd]" id="price_min_sq_yrd" placeholder="Price Range (Min.) Rs./sq.yd" value="<?=$dbland[0]['price_min_sq_yrd']?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                        <br>
                        <br>
                      </div>
                      <div class="col-md-6">
                        <h6>Price Range (Max.) Rs./sq.yd</h6>
                        <input class="form-control" type="text" name="info[price_max_sq_yrd]" id="price_max_sq_yrd" placeholder="Price Range (Max.) Rs./sq.yd" value="<?=$dbland[0]['price_max_sq_yrd']?>" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                        <br>
                        <br>
                      </div>
                    </div>
...
function ckhform(){
	...
	if(isEmpty("land",document.getElementById("land").value)){
