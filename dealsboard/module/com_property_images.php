...
access_check('com_property_images');
$msg = base64_decode($_REQUEST['msg'] ?? "");
$view = $dbObj->sc_mysql_escape($_REQUEST['view'] ?? "");
$id = $dbObj->sc_mysql_escape($_REQUEST['id'] ?? "");
$property_id = ($_REQUEST['property_id'] ?? "");

$var_extra = "com_property_images"; // to enable page link
...
$dbObj->dbQuery="select count(*) as total from ".PREFIX."com_property_images where property_id='".$property_id."'"; // for total number of records for paging
...
$dbObj->dbQuery="select * from ".PREFIX."com_property_images where property_id='".$property_id."'";  // for listing of records
if(empty($view)){
...
$dbObj->dbQuery="select * from ".PREFIX."com_property_images where property_id='".$property_id."' and id='".$id."'";
            ...
            <div class="card-body">
                              ...
                              <li id="recordsArray_<?=$dbPropertyImages[$i]['id']?>">
                                <table width="100%">
                                  <tr>
                                    ...
                                    <td align="center" width="8%"><a href="index.php?mo=com_property_images&property_id=<?=$dbPropertyImages[$i]['property_id']?>&id=<?=$dbPropertyImages[$i]['id']?>" class="text-primary">Edit</a></td>
                                  </tr>
                                </table>
                              </li>
                              <?php } ?>
              ...
              <div class="text-right right">
                <ul class="pagination pull-right">
                  ...
                  <a class="btn btn-primary" href="index.php?mo=com_property_images&property_id=<?=$property_id?>&view=all">View All</a>
                </ul>
              </div>
              <?php } ?>
            </div>
...
var settings = {
	url: "comupload.php?property_id=<?=$property_id?>",
	method: "POST",
...
$(function() {
		$("#contentLeft ul").sortable({ opacity: 0.6, handle: '#move', update: function() {
			var order = $(this).sortable("serialize") + '&action=change_property_images_order' + '&list=<?=$list?>' + '&count=<?=$cntH?>'+ '&page=<?=$page?>'+ '&page_limit=<?=$page_limit?>'+ '&property_id=<?=$property_id?>';
...
function front_status(id,value,is_check){
	...
	$.ajax({
		url:'compropertyController.php?mode=front_status',
...
function pro_image_status(id,value,is_check){
	var setval=(is_check==true)?1:0;
	$.ajax({
		url:'compropertyController.php?mode=pro_image_status',
		data:'id='+value+'&setval='+setval+'&page=<?=$page?>&set=<?=$set?>',
		success:function(response){
			//alert(response);
			//document.getElementById("msg").innerHTML = "Status Successfully Changed";
