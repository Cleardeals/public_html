...
//access_check('location');
$msg = base64_decode($_REQUEST['msg'] ?? "");
$view = $dbObj->sc_mysql_escape($_REQUEST['view'] ?? "");
$id = $dbObj->sc_mysql_escape($_REQUEST['id'] ?? "");
$var_extra = "make_offer"; // to enable page link
...
$dbObj->dbQuery="select * from ".PREFIX."make_offer"; // for listing of records
...
$dbMakeOffer = $dbObj->SelectQuery();
$cntH = count((array)$dbMakeOffer);
?>
<style>
#contentLeft {
	width: 100%;
            ...
            <div class="card-body">
              ...
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_make_offer"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbMakeOffer)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      <?php for($i=0;$i<$cntH;$i++){ 
					    $dbObj->dbQuery="select * from ".PREFIX."property where id='".$dbObj->sc_mysql_escape($dbMakeOffer[0]['property_id'])."'";
						$dbProperty = $dbObj->SelectQuery();
						
						$dbObj->dbQuery="select * from ".PREFIX."property_detail where property_id='".$dbObj->sc_mysql_escape($dbMakeOffer[0]['property_id'])."'";
						$dbPropertyDetail = $dbObj->SelectQuery();
					  ?>
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbMakeOffer[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbMakeOffer[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=$dbMakeOffer[$i]['name']?></td>
                        <td width="10%"><?=$dbMakeOffer[$i]['email']?></td>
                        <td width="10%"><?=$dbMakeOffer[$i]['mobile_no']?></td>
                        ...
                        <td width="10%"><?=$dbMakeOffer[$i]['offer']?></td>
                        <?php
							$datef = $dbMakeOffer[$i]['current_datetime'];
