...
$dbObj->dbQuery="select * from ".PREFIX."date_time"; // for listing of records
...
$dbAppointment = $dbObj->SelectQuery();
$cntH = count((array)$dbAppointment);
?>
<style>
#contentLeft {
	width:100%;
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <?php } ?>
              <h3>Appointment by Cleardeals</h3>
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_app"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbAppointment)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
					  ...
					  $dbObj->dbQuery="select * from ".PREFIX."valuation where id=".$dbObj->sc_mysql_escape($dbAppointment[$i]['pid'])."";
					  $dbUsers = $dbObj->SelectQuery();
					  ?>
                      <tr>
                        <td class="text-center" width="5%">
                        <span id="<?=$dbAppointment[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbAppointment[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=date('d/m/Y',strtotime($dbAppointment[$i]['current']))?></td>
                        <td width="10%"><?=$dbUsers[0]['name'] ?? ""?></td>
                        <td width="10%"><?=$dbUsers[0]['email'] ?? ""?></td>
                        <td width="10%"><?=$dbUsers[0]['mobile_no'] ?? ""?></td>
                        <td width="10%"><?=$dbUsers[0]['city'] ?? ""?></td>
                        <td width="10%"><?=$dbUsers[0]['location'] ?? ""?></td>
                        <td width="10%"><?php $property_type = $dbUsers[0]['property_type'] ?? ""; if($property_type==1){?>
                          Flat
                          <?php }else{?>
                          Superbuilt Up Area
                          <?php }?></td>
                        <td width="10%"><?php $sqf = $dbUsers[0]['sqf'] ?? ""; if($sqf==1){?>
                          <?=$dbUsers[0]['area'] ?? ""?>
                          sq.Feet
                          <?php }else{?>
                          <?=$dbUsers[0]['area'] ?? ""?>
                          sq.Yard
                          <?php }?></td>
                        <td width="10%"><?=$dbUsers[0]['est_min_value'] ?? ""?> - <?=$dbUsers[0]['est_max_value'] ?? ""?></td>
                        <?php
							$datef = $dbAppointment[$i]['p_date_time'];
							$appoin_dates = explode(" ",$datef);
							$date = $appoin_dates[0];
							$time = $appoin_dates[1];
							$dates = date('d/m/Y',strtotime($date));
						 ?>
                        <?php if($dbAppointment[$i]['p_date_time']){?>
                  ...
                  <table id="testTable" class="display nowrap table table-hover table-striped table-bordered" width="100%" style="display:none">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        ...
                        <td width="10%"><?=$dbUsers[0]['city']?></td>
                        <td width="10%"><?=$dbUsers[0]['location']?></td>
                        <td width="10%"><?php if($dbUsers[0]['property_type']==1){?>
                          Flat
                          <?php }else{?>
                          Superbuilt Up Area
                          <?php }?></td>
                        <td width="10%"><?php if($dbUsers[0]['sqf']==1){?>
                          <?=$dbUsers[0]['area']?>
                          sq.Feet
                          <?php }else{?>
                          <?=$dbUsers[0]['area']?>
