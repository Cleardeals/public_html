...
$var_extra = "states"; // to enable page link
...
$dbObj->dbQuery="select * from ".PREFIX."state where id!=''"; // for listing of records
...
$dbObj->dbQuery="select * from ".PREFIX."state where id='".$id."'";
$State = $dbObj->SelectQuery();
?>
<style>
#contentLeft {
	width: 100%;
  ...
  <div class="page-wrapper"> 
    ...
    <div class="container-fluid"> 
      ...
      <form action="contentController.php" method="post" id="accForm" enctype="multipart/form-data">
        <input type="hidden" name="mode" value="add_update_state" />
        <input type="hidden" name="id" value="<?=$id?>" />
        <div class="row" id="main">
          <div class="col-md-12" id="content">
            <div class="row accordion" id="accordion1">
              <div class="col-md-12">
                <div class="card"> <a class="card-header collapsed in card-title" data-toggle="collapse" href="#collapseOne"> Add/Update State <i class="fa accrd-controller fa-caret-down pull-right"></i></a>
                  <div id="collapseOne" class="card-body collapse show border font-13" data-parent="#accordion1">
                    <h6>State Name</h6>
                    <input class="form-control" type="text" name="info[state_name]" id="state_name" value="<?=$State[0]['state_name'] ?? ""?>">
                    <br>
                    <br>
            ...
            <div class="card-body">
              ...
              <div class="table-responsive mt-10">
                <form action="contentController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_state"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbState)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbState[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbState[$i]['id']?>">
                          </span></td>
                        <td id="move" width="20%"><?=$dbState[$i]['state_name']?></td>
                        <td width="8%"><input type="checkbox" id="s<?=$i+1?>"  <?=($dbState[$i]['status']==1)?'checked':''?> onClick="state_status(<?=$dbState[$i]['id']?>,this.value,this.checked)" value="<?=$dbState[$i]['id']?>"></td>
                        <td width="10%"><?=$dbState[$i]['display_order']?></td>
                        <td width="8%"><a href="index.php?mo=states&id=<?=$dbState[$i]['id']?>" class="text-primary">Edit</a></td>
                      </tr>
                      <?php }?>
                    </tbody>
                    <tfoot>
...
function ckhform(){
	if(isEmpty("State Name",document.getElementById("state_name").value)){
...
function state_status(id,value,is_check){
	var setval=(is_check==true)?1:0;
	$.ajax({
		url:'contentController.php?mode=state_status',
