...
$dbObj->dbQuery="select * from ".PREFIX."services"; // for listing of records
        ...
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              ...
              <div class="row">
                <div class="text-left col-md-6"> <a href="index.php?mo=add_services" class="btn waves-effect waves-light btn-rounded btn-primary"> Add New</a></div>
              </div>
              <div class="table-responsive mt-10">
                <form action="servicesController.php" method="post" id="hostForm">
                  <input type="hidden" name="mode" value="delete_services"/>
                  <input type="hidden" name="counter" id="counter" value="<?=count($dbServices)?>" />
                  <table id="myTable" class="display nowrap table table-hover table-striped table-bordered" width="100%">
                    ...
                    <tbody id="changes">
                      ...
                      <tr>
                        <td class="text-center" width="5%"><span id="<?=$dbServices[$i]['id']?>">
                          <input type="checkbox" id="c<?=$i?>" name="id[]" value="<?=$dbServices[$i]['id']?>">
                          </span></td>
                        <td width="10%"><?=$dbServices[$i]['title']?></td>
                        <td width="20%"><img src="../cms_images/services/original/<?=$dbServices[$i]['image']?>" width="100"></td>
                        <td width="5%"><input type="checkbox" id="s<?=$i+1?>" <?=($dbServices[$i]['status']==1)?'checked':''?> onClick="service_status(<?=$dbServices[$i]['id']?>,this.value,this.checked)" value="<?=$dbServices[$i]['id']?>"></td>
                        <td width="5%"><a href="index.php?mo=add_services&id=<?=$dbServices[$i]['id']?>" class="text-primary"> Edit</a></td>
                      </tr>
                      <?php }?>
                    </tbody>
                    <tfoot>
...
function service_status(id,value,is_check){
	var setval=(is_check==true)?1:0;
	$.ajax({
		url:'servicesController.php?mode=service_status',
